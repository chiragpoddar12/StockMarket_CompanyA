...
@RestController
public class TestControllerCompanyA {

	@RequestMapping(value = "/companyA", method = RequestMethod.GET)
	public Company firstPage() {
		Company cmp = new Company();
		cmp.setName("ABC");
		cmp.setNumberOfShares(1000);
		cmp.setPriceOfShares(50);
		cmp.setSharesDoublesWhen(500);
		return cmp;
	}
	@RequestMapping(value = "/companyA", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
...
//	    ResponseUpgrade response= null;
		if (response != null) {
			...
			try {
				...
				String q="Select Number_of_shares from company_a WHERE name='"+response.getSellerName()+"'";
				...
				String r="UPDATE company_a SET Number_of_shares="+seller_shares_after+" WHERE name='"+response.getSellerName()+"'";
				...
				String s="SELECT Number_of_shares from company_a WHERE name='"+response.getBuyerName()+"'";
				...
				String t="UPDATE company_a SET Number_of_shares="+buyer_shares_after+" WHERE name='"+response.getBuyerName()+"'";
